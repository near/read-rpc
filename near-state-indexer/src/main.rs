...
async fn handle_block(
    ...
    chunks: Vec<(
        database::primitives::ChunkHash,
        database::primitives::ShardId,
        database::primitives::HeightIncluded,
    )>,
    db_manager: &(impl database::StateIndexerDbManager + Sync + Send + 'static),
) -> anyhow::Result<()> {
...
async fn handle_state_changes(
    ...
    let mut state_changes_to_store = std::collections::HashMap::<
        String,
        near_indexer_primitives::views::StateChangeWithCauseView,
    >::new();
    ...
    for state_change in initial_state_changes {
        ...
        let key = match &state_change.value {
            ...
            | StateChangeValueView::ContractCodeDeletion { account_id } => {
                format!("{}_contract", account_id.as_ref())
            }
            StateChangeValueView::AccountUpdate { account_id, .. }
            | StateChangeValueView::AccountDeletion { account_id } => {
                format!("{}_account", account_id.as_ref())
            }
        };
...
async fn store_state_change(
    ...
    match state_change.value {
        ...
        StateChangeValueView::AccessKeyUpdate {
            ...
            let add_access_key_future = db_manager.add_access_key(
                account_id.clone(),
                block_height,
                block_hash,
                &data_key,
            ...
            {
                let add_account_access_keys_future = db_manager.add_account_access_keys(
                    account_id,
                    block_height,
                    &data_key,
                    Some(&data_value),
        ...
        StateChangeValueView::AccessKeyDeletion {
            ...
            let data_key = public_key
                .try_to_vec()
                .expect("Failed to borsh-serialize the PublicKey");
            let delete_access_key_future = db_manager.delete_access_key(
                account_id.clone(),
                block_height,
                block_hash,
                &data_key,
            ...
            let mut handlers = tokio_stream::wrappers::ReceiverStream::new(stream)
                .map(|streamer_message| {
                    handle_streamer_message(
                        ...
                        &db_manager,
                        &rpc_client,
                        main_indexer_config.clone(),
                        std::sync::Arc::clone(&stats),
                    )
                })
                .buffer_unordered(main_indexer_config.general.concurrency);
