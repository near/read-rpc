...
impl crate::TxIndexerDbManager for PostgresDBManager {
    ...
    async fn cache_add_transaction(
        ...
        crate::models::TransactionCache {
            block_height: bigdecimal::BigDecimal::from(transaction_details.block_height),
            transaction_hash: transaction_details.transaction.hash.clone().to_string(),
            transaction_details: borsh::to_vec(&transaction_details)?,
        }
        .insert_or_ignore(Self::get_connection(&self.pg_pool).await?)
        .await?;
        Ok(())
