...
impl crate::StateIndexerDbManager for MockDBManager {
    ...
    async fn add_validators(&self, epoch_id: CryptoHash, epoch_height: u64, epoch_start_height: u64, validators_info: &EpochValidatorInfo) -> anyhow::Result<()> {
        todo!()
    }

    async fn update_epoch_end_height(&self, epoch_id: CryptoHash, epoch_end_block_hash: CryptoHash) -> anyhow::Result<()> {
        todo!()
    }

    async fn save_block(&self, block_height: u64, block_hash: CryptoHash, chunks: Vec<(ChunkHash, ShardId, HeightIncluded)>) -> anyhow::Result<()> {
        todo!()
    }

    async fn save_state_change(&self, state_change: &StateChangeWithCauseView, block_height: u64, block_hash: CryptoHash) -> anyhow::Result<()> {
        todo!()
    }
}
